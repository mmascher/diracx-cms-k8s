...
[github.com/DIRACGrid/diracx-charts](https://github.com/DIRACGrid/diracx-charts) 
and is the recommended path for production deployments.

Before you start a few things should be done:
- decide on domain name, e.g. for local deployment I use
`https://kubernetes.docker.internal/`. It can be easily defined
using the following lines in your `/etc/hosts` file:
```
cat /etc/hosts
...
# add this for local docker desktop
127.0.0.1 kubernetes.docker.internal
```
- register your application in CERN application portal, e.g.
visit `https://application-portal.web.cern.ch/` and register new application
like `cms-diracx-local`. You need to use your kubernetes hostname
in Redirect URI(s) and Base URL as well as choose Public Client
option for your client secret configuration.
- construct your values file, e.g. I use this name `values-localhost.yaml`
for my local deployment. You can find example of this file in this
repository.
- finally, you **MUST** decide how to supply to your deployment
DiracX configuration. You have the following options:
  - use local file, for that you can place it under `helm/files/default.yml`.
  Please note, that the file name should be exactly like that with that
  extention as it is hardcoded within DiracX codebase, see it
  [here](https://github.com/DIRACGrid/diracx/blob/main/diracx-core/src/diracx/core/config/sources.py#L29)
  - use private github/gitlab repo where you place your configuration
  and configure using it

DiracX provides different way to configure usage of configuration
file:
- using local file, please use this configuration in your config file
```
csConfig:
